...
每次循環付費授權成功續訂時，你都需要讓 Liberation 連線到網際網路，以更新其內部授權。因此，如果是每月自動續訂的授權，你就需要每個月連線一次。

#### **如果下一次付款後我的電腦無法連線到網際網路，會發生什麼事？**

對於每月循環付費授權，Liberation 通常會在付費授權續訂後提供 7 天寬限期，讓你連線到網際網路並更新內部授權。超過這段時間後，Liberation 會回到 _Free_ 模式。

#### **如果我的信用卡過期會怎麼樣？**

你會收到我們付款服務供應商寄出的電子郵件通知，並需要更新信用卡資料。登入網站後，在授權頁面使用 _UPDATE CARD DETAILS_，或在 _Billing and payments_ 下使用 _Update_。你必須在寬限期內完成，以免失去付費功能的存取權。
...
你可以透過網站取消該電腦的授權。如果該 Liberation 安裝在上次授權重新整理後未曾上線，就可以立即完成。

如果不是，取消授權會在授權下次重新整理時，或該電腦連線到網際網路時生效，以較早發生者為準。如果你急需重新授權一台新電腦，請聯絡支援團隊。
...
主要設定是 _Scanner Sync_，它會補償掃描鏡移動與雷射亮度變化之間的輕微延遲。如果你的雷射點或光束有一點「尾巴」，就需要調整這個設定。（請參閱 [雷射輸出設定面板](setting-up/laser-settings.md "mention")頁面上的照片，了解「尾巴」的範例）
