...
每個 Liberation 版本都有自己的工作資料夾。例如，如果你使用的是 1.0.0 版，資料夾名稱就會是 1.0.0。
...
1. **備份**整個版本資料夾：
   * macOS：在 `1.0.0` 資料夾上按右鍵，選擇 **Compress** 建立 zip 檔，或將它複製到安全的位置，例如桌面。
   * Windows：在 `1.0.0` 資料夾上按右鍵，選擇 **Send to > Compressed (zipped) folder**，或將它複製到安全的位置，例如桌面。
2. 備份完成後，從 Liberation 工作位置**刪除**原本的 `1.0.0` 資料夾。
...
3. 如果仍然打不開，請先備份再刪除整個 `1.0.0`（或你的版本）資料夾。
