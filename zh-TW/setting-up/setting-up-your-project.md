...
第一次開啟 Liberation 時，系統會使用預設配置。不過，你很可能會想調整這些設定，讓它更符合你自己的設定方式。
...
3.  **調整 zones：**\
    在 _OUTPUT_ view 中，你可以檢查 zones。使用 Tab 在各台雷射之間切換，或按一下檢視畫面上方的其中一個編號按鈕。依需求調整每個 zone，甚至可以新增 zones。請參閱 [Output view](../output-view/ "mention")。

    <figure><img src="../.gitbook/assets/Laser Output View.png" alt="" width="375"><figcaption></figcaption></figure>
4.  **變更每個 Clip 的 zones：**\
按一下 Clip 的按鈕來觸發它，然後使用畫面上的按鈕切換各個 zone，以及 X 與 Y 翻轉。請參閱 [Clips 與 Clip Deck](../clips/ "mention")。
