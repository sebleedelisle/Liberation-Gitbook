...
若要與 Ableton Link 同步，請選擇 _ABLETON LINK_ 作為 tempo 來源。Liberation 會加入本機網路上的 Link session，並跟隨其他支援 Link 的應用程式所共享的 tempo 與拍相位。

Ableton Link 不使用 MIDI 連接埠，也不傳送絕對歌曲位置。如果需要讓 Liberation 的小節起點對齊演出中的特定時刻，請使用小節重設控制。
