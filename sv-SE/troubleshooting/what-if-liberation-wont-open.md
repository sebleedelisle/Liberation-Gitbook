...
Varje version av Liberation har en egen arbetsmapp. Om du till exempel kör version 1.0.3 heter mappen 1.0.3.
...
2.  Klistra in den här sökvägen och tryck på **Enter**:
    ...
    ```
3. Öppna mappen som matchar ditt versionsnummer, till exempel `1.0.3`.
...
1.  Tryck på **Win + R**, klistra in detta och tryck på **Enter**:
    ...
    ```
2. Öppna mappen som matchar ditt versionsnummer, till exempel `1.0.3`.
...
* **Exempel för macOS**: `~/Library/Application Support/Liberation/1.0.3/data/liberation/settings.json`
* **Exempel för Windows**: `%LOCALAPPDATA%\Liberation\1.0.3\data\liberation\settings.json`
...
* macOS: Högerklicka på mappen `1.0.3` och välj **Compress** för att skapa en zip-fil, eller kopiera den till en säker plats, till exempel Desktop.
* Windows: Högerklicka på mappen `1.0.3` och välj **Send to > Compressed (zipped) folder**, eller kopiera den till en säker plats, till exempel Desktop.
2. När du har säkerhetskopierat tar du bort den ursprungliga mappen `1.0.3` från Liberations arbetsplats.
...
* Liberation-version, till exempel 1.0.3
...
* **Bilaga**: den zippade säkerhetskopian av din `1.0.3`-arbetsmapp.
...
3. Om det fortfarande inte öppnas, säkerhetskopiera och ta sedan bort hela mappen `1.0.3` eller motsvarande mapp för din version.
