...
Het belangrijkste type zone dat je voor de meeste projecten gebruikt, is de _Beam zone_. Dit is een zone die is bedoeld voor atmosferische beam-effecten door de lucht. Het andere type zone is een _Canvas zone_ (zie [Graphics en het Canvas-systeem](../graphics-and-the-canvas-system/)).

{% hint style="danger" %}
**WAARSCHUWING - Wees uiterst voorzichtig wanneer je zones verplaatst terwijl de laser actief is** en zet de helderheid zo laag mogelijk. Zie [Overzicht van het instelproces voor lasers](../setting-up/setting-up-lasers.md) voor een uitgebreide gids om lasers veilig te activeren en zones in te stellen.
{% endhint %}
...
{% hint style="info" %}
Top tip: je kunt zone-instellingen snel naar meerdere lasers kopiëren! Zie [Instellingen kopiëren tussen lasers](../setting-up/copy-laser-settings.md)
{% endhint %}

### Een nieuwe beam-zone toevoegen

Klik op de knop _Add a new beam zone_ bovenaan de toolbar en er verschijnt een nieuwe zone. Let op: beam zones worden gesorteerd in de volgorde waarin je ze toevoegt, maar je kunt die volgorde aanpassen. Zie [Beam zones opnieuw ordenen](re-ordering-beam-zones.md)

### Een bestaande canvas-zone toevoegen

Klik op de knop _Add existing canvas zone_. Je ziet dan een lijst met beschikbare canvas zones, die je voor deze laser aan en uit kunt zetten. Zie [Graphics en het Canvas-systeem](../graphics-and-the-canvas-system/)
...
* De zone verwijderen
* Een _Alt Zone_ toevoegen (zie [Alt zone-systeem](alt-zone-system.md))
