...
Il est particulièrement efficace d’appliquer un zone delay à un Clip déjà en cours de lecture, puis d’utiliser la commande correspondante sur l’APC40 pour ajuster le niveau et le motif. (Voir [Référence APC40](../reference/apc40-reference.md)). Vous pouvez aussi utiliser le panneau _Clip Settings_.
...
Lorsqu’un Clip utilise un _Zone delay_, vous verrez une icône à trois points en haut à droite du Clip. Ces points sont animés pour vous indiquer le style de _Zone delay_ utilisé par ce Clip. Voir [Que signifient les petites icônes sur les boutons de Clip ?](what-are-the-small-icons-on-the-clip-buttons.md) pour plus de détails.
