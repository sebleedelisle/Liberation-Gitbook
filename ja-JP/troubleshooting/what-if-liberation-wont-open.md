...
Liberation の各バージョンには、それぞれ専用の作業フォルダーがあります。たとえば、バージョン 1.0.3 を使用している場合、フォルダー名は 1.0.3 になります。
...
2.  次のパスを貼り付けて **Enter** を押します。
    ...
    ```
3. 使用しているバージョン番号と一致するフォルダーを開きます。例：`1.0.3`
...
1.  **Win + R** を押し、次を貼り付けて **Enter** を押します。
    ...
    ```
2. 使用しているバージョン番号と一致するフォルダーを開きます。例：`1.0.3`
...
* **macOS の例**: `~/Library/Application Support/Liberation/1.0.3/data/liberation/settings.json`
* **Windows の例**: `%LOCALAPPDATA%\Liberation\1.0.3\data\liberation\settings.json`
...
* macOS: `1.0.3` フォルダーを右クリックして **Compress** を選択し zip を作成するか、Desktop など安全な場所にコピーします。
* Windows: `1.0.3` フォルダーを右クリックして **Send to > Compressed (zipped) folder** を選択するか、Desktop など安全な場所にコピーします。
2. バックアップ後、Liberation の作業場所から元の `1.0.3` フォルダーを**削除**します。
...
3. それでも開かない場合は、`1.0.3`（または使用中のバージョン）フォルダー全体をバックアップしてから削除します。
