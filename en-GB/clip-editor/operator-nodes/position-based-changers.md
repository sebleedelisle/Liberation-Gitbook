...
* **Colour Changer by Position** – applies a gradient across the chosen axis or around the radial angle.\
...
These determine which aspects of the colour adjustments are applied to the content. See also: [Colour settings and HSB](../fundamentals/colour-settings-and-hsb.md).
...
Uses the same gradient editor as [Colour change](colour-changer.md), but it maps the gradient across the content by position.
...
6. Set **hue mode** OFF.
7. Set **saturation mode** FIXED if you want to force the result to greyscale.
