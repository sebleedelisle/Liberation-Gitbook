...
更多詳情請參閱 [Output View](output-view/)。
...
為了幫助你掌握位置，上方有一個 clip deck 的 mini visualiser。另請參閱 [Clips](clips/)
...
當你啟動一個 clip，它會亮起表示正在運行。它亦會有白色外框，表示這是目前 _selected_ clip。每當你切換 zone buttons 或調整 clip settings，這些操作都會套用到 _目前選取的 clip_。
...
更多詳情請閱讀 [Controller Assignment](setting-up/controller-assignment.md) 章節。
