...
{% hint style="warning" %}
重要：在啟用任何雷射輸出之前，請先確保你了解相關風險，並仔細閱讀[雷射設定流程概覽](../setting-up/setting-up-lasers.md "mention")章節。
{% endhint %}
...
任何時候運行雷射，都必須備有**硬件緊急停止按鈕**（請參閱[緊急停止／互鎖](../hardware/emergency-stop-interlocks.md "mention")）。但如果你只是想以較不緊急的方式停用所有輸出，可以使用 _**DISARM ALL**_ 按鈕，或按 `Escape` 鍵（或 APC40 上的 _**SESSION**_ 鍵）。你亦可以使用畫面上的 slider，或 APC40 的 main fader，降低全域亮度。
...
* **Scanner sync**（有時稱為 _blank shift_，之前稱為 Colour Shift）Scanner 會非常快速地移動雷射，但亮度和顏色的變化通常會與移動不同步。這會在光束和線條邊緣顯示為細小閃爍的光「尾巴」。使用此調整可令移動和顏色彼此同步。請參閱 [雷射輸出設定面板](../setting-up/laser-settings.md "mention")

其他進階 scanner settings 會在[進階](../advanced/ "mention")章節中說明。

### Zoning

如需設定雷射及劃分 zone 的完整指南，請參閱：[雷射設定流程概覽](../setting-up/setting-up-lasers.md "mention")
