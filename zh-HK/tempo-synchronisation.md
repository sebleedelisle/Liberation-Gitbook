...
它亦能判斷是否有兩個人同時在 tap tempo（即是一個用鍵盤、另一個用 APC40）。Liberation 會將重複點按平均處理。
{% endhint %}
...
要與 Ableton Link 同步，請選擇 _ABLETON LINK_ 作為 tempo source。Liberation 會加入你本機網絡上的 Link session，並跟隨其他支援 Link 的應用程式所共享的 tempo 和 beat phase。

Ableton Link 不會使用 MIDI 連接埠，亦不會傳送絕對歌曲位置。如果需要讓 Liberation 的小節起點對齊演出中的特定時刻，請使用小節重設控制。
