...
{% hint style="info" %}
If you can't see the canvas buttons in the clip deck, try scrolling the beam zone buttons with `Shift + Left / Right Arrow`. You should see a button for every canvas target area labeled _CANVAS 1, CANVAS 2_ etc.
{% endhint %}
...
{% hint style="info" %}
You can also assign a canvas zone to a laser by clicking the _add canvas zone_ button in the _OUTPUT_ view. See [Zones](../output-view/zones.md "mention").
{% endhint %}

### Guide images

You can add a guide image into the canvas, and use this as a template for your graphics. It's advisable to adjust the color tint on the guide image (right-click menu) and darken it down to more easily see your content over it.
...
### Canvas in the 3D visualizer

It would probably be tricky (to say the least) to recreate your complicated multi-laser projection system in the 3D visualizer! So instead, you have the option to place your canvas within the 3D space. Activate the _Show canvas_ checkbox in the _3D visualizer settings_ panel. (Any guide images you have in the canvas will also show up in the visualizer.)

{% hint style="info" %}
Note that the visualizer will still show the canvas projections as atmospheric effects coming from the lasers. You can either just move them out the view or, if you're fancy, you can line them up with the canvas!
{% endhint %}

<figure><img src="../.gitbook/assets/Screenshot 2025-01-17 at 10.36.49.png" alt=""><figcaption><p>It can be extremely satisfying when you line up the beams from the laser with the canvas image in the 3D visualizer!</p></figcaption></figure>
