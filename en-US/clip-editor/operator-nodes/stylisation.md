...
* **change render profile for trail** – if on, all trail copies use the selected **render profile**. _See_ [Render profile](../fundamentals/render-profile.md).
...
* **separation** – how different neighboring points/elements are from each other.
...
* **Z** – roll clockwise/counterclockwise (like tilting your head sideways)
...
* **hue start** – initial color of particles.
* **hue variation** – adds randomness so particles don’t all start with the same color.
* **hue change** – shifts hue across the particle’s lifetime, creating color-changing trails.
...
* **saturation start / saturation end** – sets how vivid the color is at the start and end.
