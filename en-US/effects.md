...
You can choose which groups are affected by the effect. Right-click and toggle the group checkboxes labeled _Apply to groups._
...
You could also use it to live apply 2 different color changes to 2 groups of lasers at once. Create two color change effects and select which clip groups each one is applied to.

### MX group

Short for _Mutually Exclusive_ this is a way to group effects together in such a way that only one effect in the group can be active at the same time. Notice how only one of the default color changing effects can be active at once. This is because they are all in MX Group 1.
