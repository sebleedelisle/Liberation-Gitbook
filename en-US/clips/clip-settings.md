...
_Zone Delay/Chase_ is such a fun feature it gets its own section. [Zone delay / chase](zone-delay-chase.md)
...
If you right-click on a clip, a menu appears with some of the options for that clip. See [Introduction to the Clip Editor](../clip-editor/clip-editor-intro.md), [Clip settings](clip-settings.md) and [Clip groups](groups.md) for more about the first few items in this menu.
...
If _Retrigger_ is not enabled, the clip will always be in time - it's as though the clip was started at the very beginning of the clock. It's good for when you are synchronized perfectly with the music via an external clock signal.
