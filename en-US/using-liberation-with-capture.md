...
Liberation supports [Capture](https://capture.se) as an external visualizer (version 1.0.3 onwards). If you’re already using Capture in your workflow, you can use it to visualize Liberation’s live laser output in your 3D scene.
...
Once armed, Capture will visualize the live output stream from Liberation. If a laser is disarmed in Liberation, it will remain visible in Capture as a source, but it won’t output anything.
...
### License limits and armed lasers

Capture connections are treated exactly the same as physical laser outputs.

That means:

* You can only arm as many lasers as your license tier allows
...
Liberation includes a built-in 3D visualizer, which is always available and doesn’t depend on your license tier. You can design and preview shows directly inside Liberation without any external software.
