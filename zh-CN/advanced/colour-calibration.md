...
使用 Test Pattern 进行校准（见 [测试图案](../output-view/test-patterns.md)）
...
每个图案会显示四条移动的线。激光系统中的亮度不仅受功率水平影响，也受光束停留在同一位置的时间影响。扫描器移动得越快，光束在每个点上的停留时间就越短，看起来也就越暗。

为了解决这一点，测试图案会同时缩放亮度和速度：
...
由于亮度与速度同时缩放，这些线应该看起来亮度一致。如果某条线看起来更亮或更暗，请调整相应滑块直到一致。

每个测试图案还有一条 **0% 亮度**的第五条线，正常情况下不可见。它用于修正激光在极低亮度下不出光的问题。如果你的激光在低亮度下仍然不可见，请逐步提高 **0% setting**，直到线条刚好可见，再略微回调使其再次消失。目标是找到激光开始点亮的阈值，并保持略低于它——这样淡入会更自然，不会在底部范围被截断。
