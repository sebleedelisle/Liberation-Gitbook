...
每次循环付费许可证成功续费后，都需要将 Liberation 连接到互联网以更新其内部许可证。因此，如果是按月自动续费的许可证，则每月需要连接一次。

#### **下次付款后无法将电脑连接到互联网会怎样？**

对于按月循环付费许可证，Liberation 通常会在付费许可证续费后提供 7 天宽限期，让你连接互联网并更新其内部许可证。超过该期限后，Liberation 将回到 _Free_ 模式。

#### **信用卡过期会怎样？**

你会收到支付服务商的邮件通知，需要更新银行卡信息。登录网站，在许可证页面使用 _UPDATE CARD DETAILS_，或在 _Billing and payments_ 下使用 _Update_。你必须在宽限期内完成此操作，以避免失去付费功能的访问权限。
