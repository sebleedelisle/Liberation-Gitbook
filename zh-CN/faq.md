...
Liberation 围绕流行的 APC40 Mk2 MIDI 控制器设计并优化，也支持 APC40 Mk1。参见 [使用 APC40 进行现场控制](midi-control/live-control-with-the-apc40.md "mention")
...
对于其他控制器，请使用 MIDI Send/Receive 系统，或使用能够发送 Liberation 默认 MIDI 消息的 MIDI 转换器。可在[论坛](https://forum.liberationlaser.com)搜索此设置的建议，但现实中 APC40 Mk2 对大多数现场演出而言仍是最佳选择。
...
每次付费许可证在付款后刷新时，都需要将 Liberation 连接到互联网以更新其内部许可证。如果是按月自动续费的许可证，则每月需要连接一次。

#### **下次付款后无法将电脑连接到互联网会怎样？**

付费许可证刷新后，Liberation 会提供 7 天宽限期，让你连接互联网并更新其内部许可证。超过该期限后，Liberation 将回到 _Free_ 模式。

#### **信用卡过期会怎样？**

你会收到支付服务商的邮件通知，需要更新银行卡信息。登录网站，在许可证页面使用 _UPDATE CARD DETAILS_，或在 _Billing and payments_ 下使用 _Update_。

#### **如何取消许可证的自动续费？**

登录网站，打开 [_Your licences_](https://liberationlaser.com/account/my-products) 页面，选择要管理的许可证，然后点击 _CANCEL PAYMENTS_。你的付费许可证会保持有效直到当前付费周期结束。之后不会再扣款。
...
可以在网站上取消该电脑的授权。如果该 Liberation 安装自上次许可证刷新后一直未联网，可以立即取消。

否则，取消授权会在许可证下次刷新时或该电脑连接互联网时生效，以先发生者为准。如果你需要紧急重新授权新电脑，请联系支持。
...
它有一个智能的 “tap tempo” 系统，使用方式与预期一致；你也可以使用外部 MIDI 时钟或 Ableton Link。参见 [Tempo / synchronisation](tempo-synchronisation.md "mention")。Timeline 还可以通过任意音频接口输入的 LTC/SMPTE timecode 同步。参见 [时间码](timecode.md "mention")。
