---
metaLinks:
  alternates:
    - >-
      https://app.gitbook.com/s/MdbbIbIwHdJwkEREnJyv/clips/organising-your-clip-deck
---

# 🟩 整理 Clip Deck

### 移动 / 复制 Clips

点击并拖动可移动 Clip。如果选中了多个 Clip（见 [启动 / 停止 Clip](starting-stopping-clips.md "mention")），它们会一起移动。按住 `Alt / Option` 可复制它们。将 Clip 放到空白位置即可。

{% hint style="info" %}
如果要选择大量 Clips，可以点击并拖拽套索，然后在保持拖拽的同时滚动 Clip Deck（使用方向键、APC40 或触控板/鼠标的横向滚动）。套索会延伸到 Clip Deck 边缘之外。
{% endhint %}

### 删除 Clips

删除单个 Clip：使用右键菜单。

删除多个 Clips：选中它们（`Alt / Option + Shift`-点击或拖拽套索），然后将它们拖出 Clip Deck。

### 添加空白区域

右键点击某个 Clip 按钮，选择 _ADD EMPTY COLUMN TO RIGHT_，即可在 Clip Deck 右侧添加一列空白。

### 合并多个 Clip Deck

你可以导入现有的 Clip Deck 并追加到当前 Clip Deck 的末尾。使用菜单 _File -> Clips/Effects -> Append Clip Deck_。

先选中一部分 Clips（使用套索或 `Alt / Option + Shift`-点击），然后使用 _File -> Clips/Effects -> Export Selected Clips_。这会导出仅包含所选 Clips 的 Clip Deck，随后可追加到另一个 Clip Deck。

### 按 Timeline 整理 Clips

导入 Timeline 时也会提供 Clip 选项：可以替换当前 Clip Deck、追加 Timeline 的整个 Clip Deck，或仅追加它使用到的 Clips。

如果项目中有多个 Timeline，可按 Timeline 重新整理 Clips。打开 _Timeline selection_ 面板，点击 _ORGANISE CLIPS BY TIMELINE_。随后你会看到所有 Clips 按各自 Timeline 分组。如果某个 Clip 被多个 Timeline 使用，会为每个组复制一份。
