...
当你在 Liberation 中设置好激光后，就可以将每台激光分配到实际的激光控制器。（参见 [兼容的激光与控制器（DAC）](../hardware/compatible-lasers-and-controllers-dacs.md) 以确认可用硬件。）控制器要么通过 USB 连接，要么通过网络连接。
...
* 面板分为左右两部分：左侧是激光列表，右侧是可用控制器列表。如果列表里看不到你的控制器，点击 _REFRESH_ 按钮。若仍有问题，请参见 [故障排除](../troubleshooting/)。
...
* 橙色方块 🟧 表示控制器连接不稳定，通常由网络问题引起，参见 [故障排除](../troubleshooting/)。
* 红色方块 🟥 表示无法连接控制器，参见 [故障排除](../troubleshooting/)。
