...
我用过 [M-Audio 192/6](https://www.m-audio.com/audio-midi-interfaces/air-192-6.html) 效果很好，它有耳机监听、2 个 XLR 输入，并且（至少在 macOS 上）不需要特殊驱动。若你只用于 timecode，可选择略便宜的 [M-Audio 192/4](https://www.m-audio.com/audio-midi-interfaces/air-192-4.html)（只有一个输入且无 MIDI）。总体来说，任何可靠的音频接口都应能正常工作。&#x20;
...
更建议直接买个便宜的 USB 音频接口，不建议采用这种方式。&#x20;
{% endhint %}

如果你的音频接口没有输入监听，可在 macOS 系统设置（_Sound_）中检查是否有信号。（Windows 则用 _Sound Control Panel_）。&#x20;
