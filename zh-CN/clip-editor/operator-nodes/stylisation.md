...
* **change render profile for trail** – 开启后，所有拖尾副本使用所选 **render profile**。_见_ [Render profile](../fundamentals/render-profile.md)。
...
* **delay** – 拖尾副本之间的时间间隔，以 **1/64-note** 为单位。
  ...
  * 256 = 1 小节
* **trail size** – 拖尾副本数量。
* **prefill trails** – Clip 开始时立即填充拖尾历史，而不是等待回声在最初几个节拍中逐渐累积。
