...
Canvas zones 是 Canvas 内的区域，用来发送到激光。它们在 Canvas 视图中以粉色轮廓矩形显示。你可以右键点击每个 zone，并选择要分配的激光。切换到该激光的 _OUTPUT_ 视图后，会看到新 zone 出现。
...
{% hint style="info" %}
你也可以在 _OUTPUT_ 视图中点击 _add canvas zone_ 按钮，将 canvas zone 分配给激光。请参阅 [zones](../output-view/zones.md)。
{% endhint %}

### Guide images

你可以在 Canvas 中添加参考图像，并将其作为图形模板。建议在参考图像上调整颜色色调（右键菜单）并降低亮度，这样更容易看清叠在上面的内容。
...
要在 3D visualiser 中重建复杂的多激光投影系统可能非常繁琐。所以，你可以选择把 Canvas 放入 3D 空间：在 _3D visualiser settings_ 面板中勾选 _Show canvas_。（Canvas 中的参考图像也会显示在 visualiser 中。）
