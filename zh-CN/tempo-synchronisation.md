...
它还能判断是否有两个人同时在敲击 tempo（例如一个在键盘上，另一个在 APC40 上）。Liberation 会对这些重复敲击取平均值。
{% endhint %}
...
点击 _TEMPO MULTIPLIER_ 按钮或 APC40 上的 _BANK_ 按钮可切换 _Tempo Multiplier_。使用屏幕上的滑块或 APC40 A-B 滑块进行调整。也可使用 _25%, 50%, 100% 200%_ 预设按钮。

## 外部 tempo 源

### MIDI Clock

要同步到外部 MIDI clock 信号，请选择 _MIDI CLOCK_ 单选按钮，并从下拉菜单中选择 MIDI 设备。请留意下拉菜单旁的彩色状态灯：
...
要与 Ableton Link 同步，请选择 _ABLETON LINK_ 作为 tempo 源。Liberation 会加入本地网络上的 Link 会话，并跟随其他支持 Link 的应用共享的 tempo 和拍相位。

Ableton Link 不使用 MIDI 端口，也不传递绝对歌曲位置。如果需要让 Liberation 的小节起点与演出中的某个特定时刻对齐，请使用小节重置控制。
