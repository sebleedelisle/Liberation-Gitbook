...
반복 결제 유료 라이선스가 정상적으로 갱신될 때마다 Liberation을 인터넷에 연결해 내부 라이선스를 업데이트해야 합니다. 따라서 월간 자동 갱신 라이선스의 경우 매월 연결해야 합니다.

#### **다음 결제 후 컴퓨터를 인터넷에 연결할 수 없으면 어떻게 되나요?**

월간 반복 결제 유료 라이선스의 경우, Liberation은 보통 유료 라이선스가 갱신된 후 7일의 유예 기간을 제공하며, 이 기간 안에 인터넷에 연결해 내부 라이선스를 업데이트할 수 있습니다. 이 기간이 지나면 Liberation은 _Free_ 모드로 돌아갑니다.

#### **신용카드가 만료되면 어떻게 되나요?**

결제 제공업체에서 이메일 알림을 받게 되며, 카드 정보를 업데이트해야 합니다. 웹사이트에 로그인한 뒤 라이선스 페이지에서 _UPDATE CARD DETAILS_를 사용하거나, _Billing and payments_ 아래의 _Update_를 사용하세요. 유료 기능에 대한 접근 권한을 잃지 않으려면 유예 기간 안에 이 작업을 완료해야 합니다.
