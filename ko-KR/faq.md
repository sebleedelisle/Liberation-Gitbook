...
Liberation은 널리 사용되는 APC40 Mk2 MIDI 컨트롤러를 기준으로 설계 및 최적화되었습니다. APC40 Mk1도 지원합니다. [APC40으로 라이브 제어](midi-control/live-control-with-the-apc40.md "mention")를 참조하세요.
...
다른 컨트롤러의 경우 MIDI Send/Receive 시스템을 사용하거나, Liberation의 기본 MIDI 메시지를 보낼 수 있는 MIDI 트랜슬레이터를 사용하세요. 이 설정에 대한 조언은 [포럼](https://forum.liberationlaser.com)에서 검색해 보세요. 하지만 현실적으로 대부분의 라이브 쇼에서는 APC40 Mk2가 여전히 가장 좋은 선택입니다.
...
* 외부 **ILDA 입력** – 호환되는 외부 컨트롤러와 함께 사용하는 25핀 D 커넥터.
* 내부에 설치된 **Ether Dream**.
...
결제 후 유료 라이선스가 새로 고쳐질 때마다 Liberation을 인터넷에 연결해 내부 라이선스를 업데이트해야 합니다. 월간 자동 갱신 라이선스의 경우 매월 연결해야 합니다.

#### **다음 결제 후 컴퓨터를 인터넷에 연결할 수 없으면 어떻게 되나요?**

Liberation은 유료 라이선스가 새로 고쳐진 후 7일의 유예 기간을 제공하며, 이 기간 안에 인터넷에 연결해 내부 라이선스를 업데이트할 수 있습니다. 이 기간이 지나면 Liberation은 _Free_ 모드로 돌아갑니다.

#### **신용카드가 만료되면 어떻게 되나요?**

결제 제공업체에서 이메일 알림을 받게 되며, 카드 정보를 업데이트해야 합니다. 웹사이트에 로그인한 뒤 라이선스 페이지에서 _UPDATE CARD DETAILS_를 사용하거나, _Billing and payments_ 아래의 _Update_를 사용하세요.

#### **라이선스 자동 갱신은 어떻게 취소하나요?**

웹사이트에 로그인하고 [_Your licences_](https://liberationlaser.com/account/my-products) 페이지를 연 다음, 관리할 라이선스를 선택하고 _CANCEL PAYMENTS_를 클릭하세요. 유료 라이선스는 현재 유료 기간이 끝날 때까지 활성 상태로 유지됩니다. 추가 결제는 이루어지지 않습니다.

#### **Liberation을 몇 대의 컴퓨터에 설치할 수 있나요?**

Liberation은 원하는 수만큼 컴퓨터에 설치할 수 있습니다. 라이선스 인증은 레이저 / DMX 출력을 활성화할 때만 필요하며, 라이선스 등급에 따라 동시에 출력용으로 인증할 수 있는 컴퓨터 수가 결정됩니다. [라이선스 작동 방식](installation/how-licensing-works.md "mention")를 참조하세요.
...
웹사이트를 통해 해당 컴퓨터의 인증을 해제할 수 있습니다. 마지막 라이선스 새로 고침 이후 해당 Liberation 설치가 온라인 상태가 된 적이 없다면 즉시 처리할 수 있습니다.

그렇지 않은 경우, 다음 라이선스 새로 고침 시점이나 해당 컴퓨터가 인터넷에 연결될 때 중 먼저 발생하는 시점에 인증 해제가 적용됩니다. 새 컴퓨터를 긴급하게 다시 인증해야 한다면 지원팀에 문의하세요.
...
예! [레이저 간 존 복사](output-view/copy-zones-between-lasers.md "mention")를 참조하세요.
...
주요 설정은 _Colour Shift_입니다. 이 설정은 미러가 움직이는 시점과 레이저 밝기가 변하는 시점 사이의 작은 지연을 보정합니다. 레이저 점/빔에 작은 '꼬리'가 보인다면 이 값을 조정해야 합니다. ('꼬리'의 예시는 [Laser output 설정 패널](setting-up/laser-settings.md "mention") 페이지의 사진을 참조하세요.)
...
이는 프레임 지연 시간, 즉 프레임이 생성된 뒤 레이저로 전송될 때까지의 최대 시간입니다. 일반적으로 조정할 필요는 없지만, 네트워크 문제가 있다면 값을 높여 볼 수 있습니다. 자세한 내용은 [지연 시간 설정](setting-up/latency-setting.md "mention")를 참조하세요.
...
해당 Clip이 사운드 또는 MIDI 입력을 받는다는 것을 표시합니다. 또한 3개의 점은 zone delay가 있음을 나타냅니다. [클립 버튼의 작은 아이콘은 무엇인가요?](clips/what-are-the-small-icons-on-the-clip-buttons.md "mention")를 참조하세요.
