...
micro USB 케이블로 LaserCube를 컴퓨터에 연결한 다음 _Controller Assignment_ 패널에서 확인합니다([컨트롤러 할당](../setting-up/controller-assignment.md "mention") 참고). 자동으로 표시되지 않으면 _REFRESH_ 버튼을 누르세요.
...
LaserCube를 "LAN Client" 모드로 설정하고 네트워크에 라우터가 있는지 확인합니다. LaserCube는 라우터에서 IP 주소를 받아오며, 이후 _Controller Assignment_ 패널에 표시되어야 합니다([컨트롤러 할당](../setting-up/controller-assignment.md "mention") 참고).
