...
Liberation의 각 버전에는 별도의 작업 폴더가 있습니다. 예를 들어 버전 1.0.3을 실행 중이라면 폴더 이름은 1.0.3입니다.
...
2.  이 경로를 붙여 넣고 **Enter**를 누릅니다.
    ...
    ```
3. 사용 중인 버전 번호와 일치하는 폴더를 엽니다. 예: `1.0.3`.
...
1.  **Win + R**을 누르고 아래 내용을 붙여 넣은 뒤 **Enter**를 누릅니다.
    ...
    ```
2. 사용 중인 버전 번호와 일치하는 폴더를 엽니다. 예: `1.0.3`.
...
* **macOS 예시**: `~/Library/Application Support/Liberation/1.0.3/data/liberation/settings.json`
* **Windows 예시**: `%LOCALAPPDATA%\Liberation\1.0.3\data\liberation\settings.json`
...
1. 전체 버전 폴더를 **백업**합니다.
* macOS: `1.0.3` 폴더를 오른쪽 클릭하고 **Compress**를 선택해 zip 파일을 만들거나, Desktop 같은 안전한 위치에 복사합니다.
* Windows: `1.0.3` 폴더를 오른쪽 클릭하고 **Send to > Compressed (zipped) folder**를 선택하거나, Desktop 같은 안전한 위치에 복사합니다.
2. 백업한 뒤, Liberation 작업 위치에서 원본 `1.0.3` 폴더를 **삭제**합니다.
...
3. 그래도 열리지 않으면 전체 `1.0.3` 폴더(또는 사용 중인 버전 폴더)를 백업한 뒤 삭제합니다.
